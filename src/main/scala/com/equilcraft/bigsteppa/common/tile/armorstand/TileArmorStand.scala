...
class TileArmorStand
  ...
  override def getSizeInventory: Int = this.inventory.length

  override def getStackInSlot(slot: Int): ItemStack =
    if (slot >= 0 && slot < this.inventory.length) this.inventory(slot) else null
  ...
  def clearWithoutSync(): Unit = {
    this.inventory = new Array[ItemStack](slotCount)
    this.markDirty()
  }
  ...
  override def writeToNBT(tag: NBTTagCompound): Unit = {
    ...
    for (slot <- this.inventory.indices) {
      val stack = this.inventory(slot)
      if (stack != null) {
